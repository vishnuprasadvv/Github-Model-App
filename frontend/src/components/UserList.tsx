...
const UserList: React.FC<Props> = ({ onSelectUser }) => {
  ...
  return (
    ...
    <div className={'sortControls'}>
      ...
      <select className={'select'} value={sortField} onChange={(e) => handleSortChange(e.target.value)}>
      ...
      <ul className={'userList'}>
        {users.map((user) => (
          <li key={user.id} className={'userCard'}>
            ...
            <button onClick={() => onSelectUser(user.username)} className={'username'}>
              {user.username}
            </button>
            ...
            <button onClick={() => onSelectUser(user.username)} className={'view-profile-button'}>
              View Profile
            </button>
          </li>
        ))}
